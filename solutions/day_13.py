...
class Day13(object):
    def __init__(self, input_file_path):
        with open(input_file_path, "r") as f:
            buses = f.readlines()
        self.timestamp = int(buses[0])
        self.bus_ids = np.array([int(b) if b != "x" else 1 for b in buses[1].split(",")])

    def part_1(self):
        min_diff = self.timestamp
        bus_id = -1
        for b_id in self.bus_ids:
            if b_id > 1:
                diff = abs((self.timestamp % b_id) - b_id)
                if diff < min_diff:
                    min_diff = diff
                    bus_id = b_id
    ...
    def part_2(self):
        mod_res = np.array([b - i for i, b in enumerate(self.bus_ids) if b > 1])
        bus_ids = np.array([b for b in self.bus_ids if b > 1])
        mod_res[0] = 0
        return self.chinese_remainder(bus_ids, mod_res)
