...
What is the total number of distinct ways you
"""
...
class Day10(object):
    def __init__(self, input_file_path):
        with open(input_file_path, "r") as f:
            self.adapters = [int(line.strip()) for line in f.readlines()]
            self.adapters.sort()
    ...
    def part_1(self):
        ...
        for adapter in self.adapters:
            diffs[adapter - current_joltage] += 1
            current_joltage = adapter

        return reduce(lambda a, b: a * b, diffs.values())

    def part_2(self):
        solutions = {0: 1}
        for adapter in self.adapters:
            solutions[adapter] = 0
            if adapter - 1 in solutions:
                solutions[adapter] += solutions[adapter - 1]
            if adapter - 2 in solutions:
                solutions[adapter] += solutions[adapter - 2]
            if adapter - 3 in solutions:
                solutions[adapter] += solutions[adapter - 3]

        return solutions[max(self.adapters)]
