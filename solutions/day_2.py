...
class PasswordPolicy(object):
    ...
    def is_valid(self, password, version="old"):
        if version == "old":
            return self.lowest <= password.count(self.letter) <= self.highest
        else:
            # indexes are 1-indexed
            return (password[self.lowest - 1] == self.letter or password[self.highest - 1] == self.letter) and (
                password[self.lowest - 1] != password[self.highest - 1]
            )
...
class Day2(object):
    def __init__(self, input_file_path):
        def _parse_input_line(line):
            ...
            lowest_highest, letter = policy.split(" ")
            lowest, highest = lowest_highest.split("-")
            return PasswordPolicy(int(lowest), int(highest), letter), password.strip()

        with open(input_file_path, "r") as f:
            self.passwords_list = [_parse_input_line(line.strip()) for line in f.readlines()]

    def part_1(self):
        return sum(policy.is_valid(password) for policy, password in self.passwords_list)

    def part_2(self):
        return sum(policy.is_valid(password, version="new") for policy, password in self.passwords_list)
