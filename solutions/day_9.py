...
class Day9(object):
    def __init__(self, input_file_path):
        with open(input_file_path, "r") as f:
            self.code = [int(line.strip()) for line in f.readlines()]

    @staticmethod
    def contains_sum(preamble, n):
        preamble.sort()
        left = 0
        right = len(preamble) - 1
        while left < right:
            _sum = preamble[left] + preamble[right]
            if _sum < n:
                left += 1
            elif _sum > n:
                right -= 1
    ...
    def part_1(self, preamble=25):
        left = 0
        right = preamble
        while right < len(self.code):
            current = self.code[right]
            if not self.contains_sum(self.code[left:right], current):
                return current
            left += 1
            right += 1

    def part_2(self, preamble=25):
        vulnerability = self.part_1(preamble=preamble)
        left = right = 0
        _sum = self.code[left]
        while _sum != vulnerability:
            if _sum < vulnerability:
                right += 1
            elif _sum > vulnerability:
                left += 1
            _sum = sum(self.code[left:right])

        return min(self.code[left:right]) + max(self.code[left:right])
